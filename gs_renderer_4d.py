...
class Renderer:
    def __init__(self, sh_degree=3, white_background=True, radius=1, T=14):
    ...
    def prepare_render(
        ...
        for t in range(self.T):
            ...
            time = ((time.float() / self.T) - 0.5) * 2 # hack
    ...
    def prepare_render_2x(
        ...
        for t in range(self.T * 2):
            time = torch.tensor(t).to(means3D.device).repeat(means3D.shape[0],1) / 2
            time = ((time.float() / self.T) - 0.5) * 2 # hack
        ...
        means3D_deform_T, scales_deform_T, rotations_deform_T, opacity_deform_T = self.gaussians._deformation(means3D_T, scales_T, 
                                                            rotations_T, opacity_T,
                                                            time_T) #  time is not none
        self.means3D_deform_T = means3D_deform_T.reshape([self.T * 2, means3D_deform_T.shape[0]//self.T //2, -1])
        self.opacity_deform_T = opacity_deform_T.reshape([self.T * 2, means3D_deform_T.shape[0]//self.T//2, -1])
        self.scales_deform_T = scales_deform_T.reshape([self.T * 2, means3D_deform_T.shape[0]//self.T//2, -1])
        self.rotations_deform_T = rotations_deform_T.reshape([self.T * 2, means3D_deform_T.shape[0]//self.T//2, -1])
