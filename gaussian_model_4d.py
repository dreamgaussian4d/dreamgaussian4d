...
def knn_index(x, k=20):
    # x: num_points x 3
    inner = -2*torch.matmul(x, x.transpose(1, 0)) # num_points x num_points
    xx = torch.sum(x**2, dim=1, keepdim=True) # num_points x 1
    pairwise_distance = -xx - inner - xx.transpose(1, 0) # num_points x num_points
 
    idx = pairwise_distance.topk(k=k, dim=-1)[1]   # (num_points, k)
    return idx
...
class GaussianModel:
    ...
    def load_ply(self, path):
        ...
        ### construct the fixed kNN graph
        self._knn_index = knn_index(self._xyz) # num_points x k
